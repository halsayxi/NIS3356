...
def run_main(tree, run_button, back_button, text_widget, entry):
    # 你的main函数
    def main():
        topic_id = entry.get()
        print(f"提交的topicID是：{topic_id}")
...
def create_gui():
    ...
    run_button = tk.Button(window, text="提交")
    run_button.pack()
    ...
    # 设置运行按钮的命令
    run_button.config(command=lambda: run_main(tree, run_button, back_button, text_widget, url_entry))
